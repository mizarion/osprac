...
/*
 ...
 Parent:
	 for(N)
		 увеличивает sem на 1 -> записывает сообщение в pipe.
		 увеличивает sem на 2 -> проверяет sem -> чтение из pipe.

 Child:
	for(N)
		уменьшает sem на 2 -> читает из pipe.
		пишет в pipe -> уменьшает sem на 1
 */
